...
module.exports = {
  findAllToUser: (userID) => {
    return db
      .query(
        `SELECT "user".username, "user".userID, notification.* FROM notification INNER JOIN user ON notification.fromUserID = user.userID WHERE toUserID = $1`,
        [userID],
      )
  ...
  create: ({ type, fromUserID, toUserID }) => {
    return db
      .query(
        `INSERT INTO notification (type, toUserID, fromUserID) VALUES ($1, $2, $3)`,
        [type, toUserID, fromUserID],
      )
      .then((res) => (res.rows.length > 0 ? res.rows : null));
  },
