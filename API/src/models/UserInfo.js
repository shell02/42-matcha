...
module.exports = {
	findOneByUser: (userID) => {
		return db.query(`SELECT userInfo.*, picture.* FROM userInfo INNER JOIN picture ON userInfo.profilePicID = picture.pictureID WHERE userID = $1`, [userID])
			.then((res) => res.rows.length > 0 ? res.rows[0] : null);
	},

	findPictures: (userInfoID) => {
		return db.query(`SELECT * FROM picture WHERE userInfoID = $1`, [userInfoID])
			.then((res) => res.rows.length > 0 ? res.rows : null);
	},

	createUserInfo: (data) => {
		if (!data.userID || !data.age || !data.gender || !data.sexualPref || (!data.city && !data.latitude))
			throw new Error("Please provide at least userID, age, gender, sexualPref and city or latitude/longitude");
		...
		if (data.city) {
			...
			if (!data.latitude && !data.longitude)
				throw new Error("Please provide both latitude and longitude (or a city)");
		...
		query += ", userID)";
		return db.query(`INSERT INTO userInfo $1 VALUES (${{...params}})`, [query])
			.then((res) => res.rows.length > 0 ? res.rows[0] : null);
	},

	updateUserInfo: (data) => {
		if (!data.userID)
			throw new Error("Please provide at least userID");
		let query = "";
		...
		return db.query(`UPDATE userInfo SET $1 WHERE userID = $2`, [query, data.userID])
			.then((res) => res.rows.length > 0 ? res.rows[0] : null);
	},

	deleteUserInfo: (userID) => {
		return db.query(`DELETE FROM userInfo WHERE userID = $1`, [userID])
			.then((res) => res.rows.length > 0 ? res.rows : null);
	},
	...
	createPicture: (data) => {
		...
		if (data.path)
			...
			choice = data.url;
		return db.query(`INSERT INTO picture (userInfoID, $1)`, ["", data.userInfoID, choice])
			.then((res) => res.rows.length > 0 ? res.rows : null);
	},

	deletePicture: (pictureID) => {
		return db.query(`DELETE FROM picture WHERE pictureID = $1`, [pictureID])
			.then((res) => res.rows.length > 0 ? res.rows : null);
	}
}
